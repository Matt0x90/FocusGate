...
To automate testing with Puppeteer:
...
(async () => {
  ...
  // Note: Can't test permission prompts in Puppeteer (Chrome security)
  // Would need to mock this or test manually
  
  // Test 3: Verify domain appears in list
  await page.waitForSelector('li.domain');
  const domains = await page.$$eval('li.domain .host', 
    els => els.map(el => el.textContent)
  );
  console.log('Blocked domains:', domains);
  
  // Test 4: Test snooze button
  const snoozeBtn = await page.$('button[data-min="15"]');
  if (snoozeBtn) {
    await snoozeBtn.click();
    await page.waitForTimeout(500);
    const status = await page.$eval('#status', el => el.textContent);
    console.log('Status after snooze:', status);
  }
...
### Testing Limitations
Note that Puppeteer cannot:
- Test actual permission prompts (Chrome security restriction)
- Test cross-origin navigation blocking (would need a test server)
- Fully test incognito mode behavior

For comprehensive testing, combine Puppeteer with manual QA.
